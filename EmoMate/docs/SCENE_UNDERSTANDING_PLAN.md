...
**阶段 5：与现有系统集成** - 🔄 **67% 完成** (2/3 步骤)
- ✅ 步骤 5.1：注入场景上下文到对话系统 (2小时)
- ✅ 步骤 5.2：实现视觉问答 (1.5小时)
- ⏳ 步骤 5.3：优化性能和成本

**下一步行动**：步骤 5.3 - 优化性能和成本
...
**已完成任务**：
- ✅ 创建 useVisualQA hook 处理视觉关键词检测和分析触发
- ✅ 在 HomeScreen 中集成视觉问答功能
- ✅ 添加帧捕获回调存储最后捕获的图像
- ✅ 在用户发送消息前自动检测视觉关键词
- ✅ 检测到视觉关键词时立即触发场景分析
- ✅ 分析结果自动更新到 userStore,AI 自动使用最新场景上下文
- ✅ 更新 personality.ts 添加视觉问答行为指导
...
**关键文件**：
- `src/utils/useVisualQA.ts:1-164` - 视觉问答hook(新建)
- `src/screens/HomeScreen.tsx:1,12,97-116,202-213,374-379` - 集成视觉问答
- `src/utils/index.ts:11` - 导出useVisualQA
- `src/constants/personality.ts:123-147` - 视觉问答指导原则
...
- [✅] 步骤 5.2：实现视觉问答 (2025-01-09, useVisualQA hook + HomeScreen集成)
- [ ] 步骤 5.3：优化性能和成本
- [ ] 步骤 6.1：端到端测试
- [ ] 步骤 6.2：用户体验优化
...
- 🔄 阶段 5 与现有系统集成：**2/3 步骤完成 (67%)**
  - ✅ 步骤 5.1：场景上下文注入完成（场景提示词生成 + 对话系统集成 + 人格系统更新）
  - ✅ 步骤 5.2：视觉问答完成（useVisualQA hook + 关键词检测 + 自动触发分析）
  - ⏳ 步骤 5.3：性能优化待实现
- 🚀 **下一步**：步骤 5.3 - 优化性能和成本
